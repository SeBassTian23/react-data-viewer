    ...
    <>
      ...
      {state && <>
        <Card.Body className='p-0 overflow-y'>
          <CalculateWilcoxonSignedRankTest {...props} parameterName={parameterName} subsets={subsets} thresholds={thresholds} />
        </Card.Body>
      </>}
    </>
  )
      ...
      {table.length === 0 &&
        <div className='d-flex justify-content-center align-items-center m-0 p-3 h-100'>
          <span className='text-danger small'>
            Wilcoxon Signed Rank Test for selected subsets and "{parameterName}" failed.
          </span>
        </div>
      }
      {table.length > 0 &&
