...
export default function WelchesTTestPanel(props) {
  return (
    <PanelStatistics 
      widgetType="welchsttest" 
      props={props} 
      CalculateComponent={CalculateWelchsTTest}
    />
  );
}

function CalculateWelchsTTest(props) {
  ...
  useEffect( () => {
    ...
    for (let i in combinations) {
      let test = welchsTTest(data[combinations[i][0]], data[combinations[i][1]], alternative)
      table.push(
        {
    ...
    <>
      ...
      {results.length > 0 && <>
        {results.map((itm, idx) => <Fragment key={idx}>
          <Table size='sm'>
            ...
            <tbody className='text-center small align-middle'>
              ...
              <tr>
                ...
                <td className={`${itm.pValue < props.confidence_level ? 'text-success' : 'text-danger'}`}>{itm.pValue < props.confidence_level ? '< ' + props.confidence_level : numberFormat(itm.pValue, 4)}</td>
              </tr>
              <tr>
                <th className='text-start'>Degrees Of Freedom</th>
                <td>{numberFormat(itm.degreesOfFreedom)}</td>
              </tr>
              <tr>
                <th className='text-start'>Sample Means</th>
                <td>{numberFormat(itm.means[0])} - {numberFormat(itm.means[0])}</td>
              </tr>
              <tr>
                <th className='text-start'>Standard Error</th>
                <td>{numberFormat(itm.standardError, 4)}</td>
              </tr>
              <tr>
          ...
          <p className='small px-2' key={idx+'i'}>{interpretWelchTTest(itm, confidenceLevel)}</p>
        </Fragment>)}
      </>
      }
    </>
