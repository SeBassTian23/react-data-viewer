...
export default function WelchesTTestPanel(props) {
  ...
  const widget = widgets.find(itm => itm.type == 'welchsttest');
    ...
    <>
      ...
      {state && <>
        <Card.Body className='p-0 overflow-y'>
          <CalculateWelchsTTest {...props} parameterName={parameterName} subsets={subsets} thresholds={thresholds} />
        </Card.Body>
      </>}
    </>
  )
...
function CalculateWelchsTTest(props) {
  ...
  for (let i in combinations) {
    let t = welchsTTest(data[combinations[i][0]], data[combinations[i][1]], 0, alternative)
    table.push(
      {
