...
export default function SpearmanCorrelationPanel(props) {
  return (
    <PanelStatistics 
      widgetType="spearmancorrelation" 
      props={props} 
      CalculateComponent={CalculateSpearmanCorrelation}
    />
  );
}
        ...
        <>
          {results.map((itm, idx) => <>
              <Table size='sm' key={idx}>
                ...
                <tbody className='small text-center'>
                  ...
                  {!itm.error && <>
                    ...
                    <tr>
                      <th className='text-start'>Correlation</th>
                      <td>{numberFormat(itm.correlation)}</td>
                    </tr>
                    <tr>
                      <th className='text-start'>p-value</th>
                      <td className={`${itm.pValue < props.confidence_level? 'text-success': 'text-danger'}`}>{numberFormat(itm.pValue)}</td>
                    </tr>
                    <tr>
                      <th className='text-start'>Confidence Interval</th>
                      <td>{numberFormat(itm.confidenceInterval[0])} - {numberFormat(itm.confidenceInterval[1])}</td>
                    </tr>
                    <tr>
                      <th className='text-start'>Sample Size</th>
                      <td>{itm.n}</td>
                    </tr>
                    <tr>
                    <th className='text-start'>Test Statistic</th>
                      <td>{numberFormat(itm.testStatistic)}</td>
                    </tr>
                  </>}
                </tbody>
              </Table>
              {!itm.error && <>
                ...
                <p className='small px-2' key={idx+'i'}>{interpretSpearmanCorrelation(itm, confidenceLevel)}</p>
              </>}
          </>)}
        </>
      }
